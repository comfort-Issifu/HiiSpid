      ...
      <div className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          {searchParams.get("search") && (
            <p className="text-amber-600 mt-2">Showing results for: "{searchParams.get("search")}"</p>
          )}
        </div>
      </div>
              ...
              {filteredItems.map((item) => (
                <Card
                  ...
                  className={`overflow-hidden hover:shadow-lg transition-all ${
                    highlightedItem === item.id ? "ring-2 ring-amber-500 shadow-lg" : ""
                  }`}
                >
                  <div className="flex">
                        ...
                        <div className="flex items-center gap-2">
                          {getCartItemQuantity(item.id) > 0 && (
                            <>
                              ...
                              <span className="w-8 text-center">{getCartItemQuantity(item.id)}</span>
                            </>
                          )}
                          <Button
                            size="sm"
            ...
            <div className="lg:w-80">
              <Card className="sticky top-4">
                <CardContent className="p-6">
                  <h3 className="text-lg font-semibold mb-4">Your Order ({getTotalItems()} items)</h3>
                    ...
                    {cart.map((item) => (
                      <div key={item.id} className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            size="sm"
                            onClick={() => handleAddToCart(menuItems.find((mi) => mi.id === item.id))}
                  ...
                  <div className="border-t pt-4">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <span className="font-bold text-lg text-amber-600">${getTotalPrice().toFixed(2)}</span>
                    </div>
